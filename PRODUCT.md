...
The application has two read-only deployment processes sharing local volumes: a web server and a periodic ingest worker. It binds to localhost by default, contains no analytics, serves no remote fonts, has no HTTP ingest endpoint, and exposes redacted health suitable for a future Omarchy badge.
