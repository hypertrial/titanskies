...
Updates are user-initiated. **Check for update** downloads the signed `macos-arm64` manifest and verifies Ed25519, architecture, minimum OS, channel, URLs, digest, and version. The installed version reports **TitanSkies is up to date** rather than an error. **Install verified update** streams the DMG to disk, hashes it incrementally, mounts it read-only, stages the new `.app`, and launches the in-bundle updater. The updater boots out both jobs, parks the current app in the rollback slot, moves the staged app into `~/Applications`, requires the service identity health check, and relaunches the new GUI. If startup fails, it restores and validates the previous app, restarts its services, relaunches it, and reports the rollback on next launch. Finder drag-replacement cannot promise automatic rollback.
