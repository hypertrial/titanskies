...
from ingest.context_sources import source_state
...
from ingest.forecast_publication import best_run_metadata, firework_run_metadata, publish_or_reuse_v8_frame
...
def demo_manifest(
    ...
    airnow_url = put_monitor_asset(store, "airnow-monitors.json", airnow_monitors)
    bc_url = put_monitor_asset(store, "bc-monitors.json", bc_monitors)
    sinaica_url = put_monitor_asset(store, "sinaica-monitors.json", sinaica_monitors)
    aqhi_url = put_monitor_asset(store, "aqhi-monitors.json", aqhi_monitors) if aqhi_monitors else None
    ...
    legend_url = put_asset(store, "forecast-legend.png", legend_png(palette_version=V8_PALETTE_VERSION), "image/png")
    firework_frames = []
    ...
    firework = firework_run_metadata(firework_run, firework_frames, ingest_method="geomet-wcs")
    hrrr = (
        {
            "modelId": "hrrr",
    ...
    best = best_run_metadata(
        model_run=composed["modelRun"],
        frames=composed["frames"],
        firework_processing_version=composed.get("fireworkProcessingVersion"),
        hrrr_processing_version=composed.get("hrrrProcessingVersion"),
        legend_url=legend_url,
        units=composed["units"],
        incomplete_numeric=composed.get("incompleteNumeric", False),
    )
    ...
    return {
        ...
        "sources": {
            name: source_state(name, now, observed, status="unavailable", error="HRRR_SMOKE_ENABLED is off")
            if name == "hrrr" and not include_hrrr
            else source_state(name, now, observed)
        ...
        "fires": {
            "wfigsIncidentsUrl": put_json_asset(store, "wfigs-incidents.json", {"incidents": us_incidents}),
            "wfigsPerimeterTextureUrl": put_asset(store, "wfigs-perimeters.png", rasterize_perimeters(us_rings), "image/png"),
            "cwfisIncidentsUrl": put_json_asset(store, "cwfis-incidents.json", {"incidents": ca_incidents}),
            "cwfisPerimeterTextureUrl": put_asset(store, "cwfis-perimeters.png", rasterize_perimeters(ca_rings), "image/png"),
        },
        "forecast": best,
...
def _demo_v8_composed(
    ...
    for hour in range(37):
        ...
        def compose(
            firework_field: NativeField = firework,
            hrrr_field: NativeField | None = hrrr,
            target_xy: tuple[np.ndarray, np.ndarray] | None = hrrr_target_xy,
        ) -> tuple[bytes, bytes, list[dict[str, Any]]]:
            return compose_v8_frame(firework_field, hrrr_field, hrrr_target_xy=target_xy)

        frames.append(
            publish_or_reuse_v8_frame(
                store,
                valid=valid,
                model_run=hrrr_run if include_hrrr else firework_run,
                contributors=contributors,
                prior=prior,
                compose=compose,
            )
        )
        del firework, hrrr, firework_values
    return {
        "units": "µg/m³",
...
def _demo_monitors(
    airnow_time: datetime, bc_time: datetime, sinaica_time: datetime, aqhi_time: datetime | None = None
    ...
    airnow = [
        ...
        item(
            "airnow", "airnow:demo-atlanta", "Atlanta", "Georgia EPD", 33.75, -84.39, 47, 11.2, airnow_time, AQI_METHOD_PROVIDER, AIRNOW_URL
        ),
        item(
            "airnow",
            "airnow:demo-washington",
            "Washington",
    ...
    bcair = [
        ...
        item(
            "bcair",
            "bcair:demo-prince-george",
            "Prince George",
            "British Columbia ENV",
            53.92,
            -122.75,
            88,
            29.4,
        ...
        item(
            "bcair",
            "bcair:demo-penticton",
            "Penticton Debeck Road",
            "British Columbia ENV",
            49.50,
            -119.60,
            58,
    ...
    aqhi = (
        []
        if aqhi_time is None
        else [
            {
                "id": f"aqhi:demo-{name.lower().replace(' ', '-')}",
                "name": name,
                "agency": "Environment and Climate Change Canada",
                "lat": lat,
                "lon": lon,
                "observedAt": iso_utc(aqhi_time),
                "indexSystem": "ca-aqhi",
                "indexValue": value,
            ...
            for name, lat, lon, value in (
                ("Vancouver", 49.2827, -123.1207, 4.4),
                ("Vancouver East", 49.2820, -123.1198, 5.1),
                ("Prince George", 53.9171, -122.7497, 7.3),
                ("Edmonton", 53.5461, -113.4938, 3.2),
                ("Toronto", 43.6532, -79.3832, 5.7),
                ("Montreal", 45.5019, -73.5674, 2.4),
                ("Halifax", 44.6488, -63.5752, 1.8),
                ("Whitehorse", 60.7212, -135.0568, 10.4),
            )
        ]
    )
...
def _demo_incidents(us_time: datetime, ca_time: datetime) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    def item(
        identifier: str, name: str, country: str, lon: float, lat: float, status: str, area: float, observed: datetime
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "areaHectares": area,
            "sourceArea": acres,
            "sourceAreaUnit": "acres" if country == "US" else "hectares",
            "updatedAt": iso_utc(observed),
            "sourceUrl": WFIGS_URL if country == "US" else CWFIS_URL,
        }

    return (
        [
            item("US:demo-1", "Pine Ridge Fire", "US", -121.45, 40.7, "Active", 8200, us_time),
            item("US:demo-2", "Canyon Fire", "US", -113.8, 43.4, "32% contained", 3100, us_time),
            item("US:demo-3", "Mesa Fire", "US", -106.2, 35.8, "Active", 1700, us_time),
        ],
        [
            item("CA:demo-1", "BC-2024-187", "CA", -122.0, 52.4, "Out of control", 12600, ca_time),
            item("CA:demo-2", "AB-2024-091", "CA", -115.6, 55.2, "Being held", 4800, ca_time),
            item("CA:demo-3", "SK-2024-044", "CA", -106.7, 57.1, "Out of control", 6900, ca_time),
        ],
    )
