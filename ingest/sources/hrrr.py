...
def decode_hrrr_massden(data: bytes) -> HrrrField:
    ...
    try:
        ...
        model_run = datetime.strptime(f"{data_date}{data_time:04d}", "%Y%m%d%H%M").replace(tzinfo=UTC)
        ...
        lat1 = (
            ...
            if eccodes.codes_is_defined(gid, "latitudeOfFirstGridPointInDegrees")
            else HRRR_LAT1
        )
        lon1 = (
            float(eccodes.codes_get(gid, "longitudeOfFirstGridPointInDegrees"))
            if eccodes.codes_is_defined(gid, "longitudeOfFirstGridPointInDegrees")
            else -122.719528
        )
        if lon1 > 180:
            lon1 -= 360
...
def planned_hours(standard: HrrrCycle, extended: HrrrCycle | None) -> list[tuple[HrrrCycle, int]]:
    ...
    if extended and standard.hours:
        ...
        jobs.extend((extended, hour) for hour in extended.hours if extended.run + timedelta(hours=hour) > last_standard_valid)
