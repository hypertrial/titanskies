...
Never put `AIRNOW_API_KEY`, `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET`, or other credentials in `NEXT_PUBLIC_*` variables, images, logs, or published data. Docker and Omarchy expose no ingest or administrative endpoint. Vercel exposes only the bearer-authenticated `/api/context` Cron function; its Blob token is restricted to server-side code and the Vercel Blob API host. Release archives and image digests are signed with the offline key corresponding to `packaging/release-cosign.pub`.
