...
def _http(status: int, body: bytes = b"", headers: dict | None = None, json_payload=None) -> Mock:
...
def _session(*, get=None, put=None, post=None) -> Mock:
    session = Mock()
...
class _MemoryBlobSession:
    ...
    def get(self, url: str, **kwargs) -> requests.Response:
        ...
        if url == "https://vercel.com/api/blob" and params is not None:
            ...
            paths = sorted(
                pathname for pathname in self.objects
                if pathname.startswith(prefix + "/") and (cursor is None or pathname > cursor)
            )
            page = paths[:limit]
        ...
        return self._response(404 if body is None else 200, body or b"")
...
def test_blob_rejected_page_cursor_is_resettable(status):
    response = requests.Response()
    response.status_code = status
...
def test_blob_page_validates_paths_and_closes_response():
...
def test_authoritative_read_never_falls_back_or_retries():
...
def test_blob_stale_takeover_deletes_only_the_observed_etag() -> None:
    conflict = _http(409)
    current = _http(200, headers={"etag": '"stale-etag"'}, json_payload={
        "owner": "stale",
    ...
    with patch("ingest.blob_store._blob_session", return_value=session):
        lease = store.acquire_lease(
            "locks/ingest.json",
            "new-owner",
            datetime(2024, 7, 15, 18, tzinfo=timezone.utc),
            datetime(2024, 7, 15, 18, 15, tzinfo=timezone.utc),
        )
...
def test_lease_create_does_not_retry_conflict() -> None:
    conflict = _http(409)
    session = _session(put=Mock(return_value=conflict), get=Mock(return_value=_http(200, json_payload={
        "owner": "other",
...
def test_blob_get_bytes_treats_forbidden_as_missing() -> None:
    ...
    assert session.get.call_count == 1
    assert "Authorization" not in session.get.call_args.kwargs["headers"]
    assert session.get.call_args.kwargs["headers"]["User-Agent"].startswith("TitanSkies/0.1.0")
...
def test_local_and_blob_storage_have_the_same_object_contract(tmp_path: Path) -> None:
    ...
    assert blob_result == local_result == {
        "state": {"version": 1},
        "immutable": b"first",
...
def test_blob_mode_requires_complete_valid_configuration(monkeypatch) -> None:
...
def test_pipeline_publishes_assets_then_manifest_then_pointer(monkeypatch, tmp_path: Path) -> None:
    class RecordingStore(LocalFrameStore):
        def __init__(self, root: Path):
            super().__init__(root)
            self.writes: list[str] = []

        def put_bytes(self, pathname, data, content_type, *, cache_seconds, overwrite):
    ...
    def build_demo(active_store, *_args, **_kwargs):
...
def test_pipeline_redacts_storage_credentials_from_failure_logs(monkeypatch, tmp_path: Path, caplog) -> None:
    def fail(*_args, **_kwargs):
        raise RuntimeError("BLOB_READ_WRITE_TOKEN=never-log-this CRON_SECRET=or-this")

    monkeypatch.setattr("ingest.context_pipeline._demo_manifest", fail)
