...
def test_demo_once_runs_ingest(monkeypatch) -> None:
...
def test_demo_watch_publishes_immediately_before_waiting(monkeypatch) -> None:
    monkeypatch.setattr("ingest.config._load_env_files", lambda: None)
    monkeypatch.setenv("CONTEXT_SOURCE", "demo")
    with patch("ingest.context_pipeline.run_context_ingest", return_value={"ok": True, "mode": "demo"}) as run:
...
def test_watch_interval_is_measured_from_ingest_start(monkeypatch) -> None:
    monkeypatch.setattr("ingest.config._load_env_files", lambda: None)
...
def test_skipped_lease_retries_without_waiting_a_full_interval(monkeypatch) -> None:
    monkeypatch.setattr("ingest.config._load_env_files", lambda: None)
    monkeypatch.setenv("CONTEXT_SOURCE", "live")
...
def test_live_once_runs_ingest(monkeypatch) -> None:
    monkeypatch.setattr("ingest.config._load_env_files", lambda: None)
    monkeypatch.setenv("CONTEXT_SOURCE", "live")
...
def test_live_once_failure_exits_nonzero(monkeypatch) -> None:
    monkeypatch.setattr("ingest.config._load_env_files", lambda: None)
    monkeypatch.setenv("CONTEXT_SOURCE", "live")
    monkeypatch.setenv("STORAGE_BACKEND", "local")
