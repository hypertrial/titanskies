...
Use Node.js 22, Python 3.11 or newer, and `uv`. Install with `npm ci` and `uv sync --all-extras`, then run `./scripts/verify-fast` before opening a pull request. Darwin arm64 Mac application changes also need `./scripts/verify-macos`.
